...
static void window_to_front_plugin_handle_method_call(
  ...
  if (strcmp(method, "activate") == 0) {
    ...
    if (view != nullptr) {
      ...
      printf("Setting window to front at this timestamp %ld\n", time);
      GtkWindow* window = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(view)));
      gtk_window_present(window);
      gtk_window_set_keep_above(window, true);
    }    
    response = FL_METHOD_RESPONSE(fl_method_success_response_new(nullptr));
  } else if (strcmp(method, "deactivate") == 0) {
    FlView* view = fl_plugin_registrar_get_view(self->registrar);
    if (view != nullptr) {
      gint64 time = g_get_real_time();
      printf("Setting window to front false %ld\n", time);
      GtkWindow* window = GTK_WINDOW(gtk_widget_get_toplevel(GTK_WIDGET(view)));
      //gtk_window_present_with_time(window, (guint32)(time));
      //gtk_window_present(window);
